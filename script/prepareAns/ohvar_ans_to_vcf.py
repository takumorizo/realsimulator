...
def prepare_vcf_header(ref_path, sample, score):
    ...
    header += '##FORMAT=<ID=LP,Number=1,Type=Float,Description=\"Lower marginal probability\">\n'
    header += '##FORMAT=<ID=UP,Number=1,Type=Float,Description=\"Upper marginal probability\">\n'
...
def main():
    ...
    pass_header = distutils.util.strtobool(argvs[6]) if len(argvs) > 6 else True
    ...
    with open(ohvar_path, 'r') as ohvar, open(output_vcf_path, 'w') as output_vcf:
        ...
        for line in ohvar:
            ...
            ref_T, obs_T, ref_N, obs_N = int(cols[5]), int(cols[6]), int(cols[7]), int(cols[8])
            BF = min_score if cols[22] == '-' else float(cols[22])
            LP = 0.0
            UP = BF
            ...
            INFO = '.'
            FORMAT_TAG = 'RCT:ACT:RCN:ACN:LP:UP:BF:HAP'
            FORMAT = ':'.join(map(str, [ref_T,obs_T,ref_N,obs_N,LP,UP,BF,HAP] ))
